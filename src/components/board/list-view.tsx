...
function TaskRow({ task, onTaskClick, statuses }: { task: TaskData; onTaskClick: (t: TaskData) => void; statuses: SpaceWithData["statuses"] }) {
  ...
  return (
    <div
      ...
      className="group flex cursor-pointer items-center gap-3 border-b border-gray-50 px-4 py-2.5 hover:bg-gray-50 transition-colors"
    >
      <div
      ...
      />
      <span className={cn("flex-1 text-sm text-gray-800 truncate", task.status.type === "done" && "line-through text-gray-400")}>
        {task.title}
      </span>
...
export function ListView({ space, tasks, onTaskClick, onTaskUpdate }: ListViewProps) {
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="sticky top-0 z-10 border-b border-gray-100 bg-white px-4 py-2">
        ...
        return (
          <div key={list.id}>
            <button
              ...
              className="flex w-full items-center gap-2 bg-gray-50 px-4 py-2 text-left text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-colors"
            >
            ...
            {!isCollapsed && (
              <>
                ...
                {listTasks.length === 0 && (
                  <div className="px-4 py-4 text-sm text-gray-400">Nenhuma tarefa</div>
                )}
              </>
            )}
          </div>
