...
export function TaskCard({ task, onClick, dragging }: TaskCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group cursor-pointer rounded-xl border border-gray-100 bg-white p-3 shadow-sm transition-all hover:shadow-md hover:border-violet-200",
    ...
    >
      <div className="mb-2 flex items-start gap-2">
        ...
        <p className="flex-1 text-sm font-medium text-gray-800 leading-snug">{task.title}</p>
      </div>

      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          {task.dueDate && (
            <span className={cn("flex items-center gap-1 rounded px-1.5 py-0.5 text-xs", isOverdue ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-500")}>
